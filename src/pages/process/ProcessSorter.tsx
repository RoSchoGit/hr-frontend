...
import { List } from "lucide-react"; // falls du "ListOrdered" willst: importiere stattdessen ListOrdered
...
type Props = {
    ...
    /**
     * render-prop: bekommst die bereits sortierten Prozesse.
     * Beispiel: <ProcessSorter processes={processes}>{sorted => ...}</ProcessSorter>
     */
    children: (sortedProcesses: Process[]) => React.ReactNode;
    /** Ref des scrollbaren Containers (für Auto-Scroll). Optional. */
    listRef?: React.RefObject<HTMLElement>;
    /** wenn true: persistiert Auswahl in localStorage (default true) */
    persist?: boolean;
    /** kleine Klasse für Positionierung */
    className?: string;
    /** icon-only: kein Label (standard true) */
    showLabel?: boolean;
    /** ob Auto-Scroll genutzt werden soll (default true) */
    autoScroll?: boolean;
};
...
export const ProcessSorter: React.FC<Props> = ({
    ...
    // Sort-Key init (aus localStorage wenn persist)
    ...
    // compute sorted list — komplett hier ausgelagert
    const sortedProcesses = useMemo(() => {
        ...
        switch (sortKey) {
            ...
            case "CREATOR_ASC":
                return items.sort((a, b) => compareStrings(a.creator, b.creator) || compareStrings(a.title, b.title));

            case "CREATOR_DESC":
                return items.sort((a, b) => compareStrings(b.creator, a.creator) || compareStrings(a.title, b.title));
    ...
    return (
        <div className={`inline-flex items-center gap-2 ${className}`}>
            ...
            >
                ...
                {/* showLabel default false — du wolltest kein Label */}
                {showLabel ? <span className="ml-2 text-sm">{SORT_OPTIONS.find(s => s.key === sortKey)?.label}</span> : null}
            </button>
            ...
            {open && (
                <select
                    ...
                    className="ml-2 bg-white border rounded p-1"
            ...
            {/* render-prop: Parent bekommt nur die bereits sortierten Prozesse */}
